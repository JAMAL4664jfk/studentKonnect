...
export default function MyMarketplaceScreen() {
  ...
  const renderListing = ({ item }: { item: MarketplaceItem }) => {
            ...
            <View className="flex-row gap-2">
              ...
              >
                <Text className="text-xs font-semibold text-primary">
                  {item.isAvailable ? "Mark Sold" : "Reactivate"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => router.push(`/edit-marketplace?id=${item.id}`)}
    ...
    <ScreenContainer>
      <View className="flex-1">
        ...
        <View className="flex-row items-center justify-between mb-6 pt-2">
          <View className="flex-1">
            <Text className="text-3xl font-bold text-primary">My Marketplace Items</Text>
        ...
        {loading ? (
          <View className="flex-1 items-center justify-center">
            <Text className="text-muted-foreground">Loading your items...</Text>
          </View>
        ) : listings.length === 0 ? (
          <View className="flex-1 items-center justify-center">
